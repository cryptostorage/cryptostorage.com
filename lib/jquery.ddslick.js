...
(function (factory) {
    ...
    var ddslickCSS = "<style id='css-ddslick' type='text/css'>" +
        ".dd-select{ border: 2.5px solid; position:relative; cursor:pointer;}" +
        ".dd-desc { color:#aaa; display:block; overflow: hidden; font-weight:normal; line-height: 1.4em; }" +
        ".dd-selected{ border: none; overflow: hidden; display: flex; display: -o-flex; display: -moz-flex; display: -ms-flexbox; display: -webkit-flex; align-items: center; -ms-flex-align: center; -webkit-align-items: center; padding:10px; line-height:35px; -webkit-user-select: none; -moz-user-select: none; -ms-user-select: none; user-select: none; }" +
        ".dd-pointer{ width:0; height:0; position:absolute; right:10px; top:50%; margin-top:-3px;}" +
        ".dd-pointer-down{ border:solid 5px transparent; border-top:solid 5px #000; }" +
        ".dd-pointer-up{border:solid 5px transparent !important; border-bottom:solid 5px #000 !important; margin-top:-8px;}" +
        ".dd-options{ border: 2.5px solid; border-top:none; list-style:none; display:none; position:absolute; z-index:2000; margin:0; padding:0; background-color:#fff; overflow:auto;}" +
        ".dd-option { padding:10px; display: flex; display: -o-flex; display: -moz-flex; display: -ms-flexbox; display: -webkit-flex; align-items: center; -ms-flex-align: center; -webkit-align-items: center; border-bottom: 2.5px solid; overflow:hidden; text-decoration:none; color:#333; cursor:pointer;-webkit-transition: all 0.25s ease-in-out; -moz-transition: all 0.25s ease-in-out;-o-transition: all 0.25s ease-in-out;-ms-transition: all 0.25s ease-in-out; -webkit-user-select: none; -moz-user-select: none; -ms-user-select: none; user-select: none; }" +
        ".dd-options > li:last-child > .dd-option{ border-bottom:none;}" +
        ".dd-option:hover{ background-color:#EBEDEF;; color:#000; }" +
        ".dd-selected-description-truncated { text-overflow: ellipsis; white-space:nowrap; }" +
        ".dd-option-selected { }" +
        ".dd-option-image, .dd-selected-image { vertical-align:middle; float:left; margin-right:10px; max-width:35px; max-height:35px;}" +
        ...
        return this.each(function() {
            ...
            if (!data) {
                ...
                $.each(options.data, function (index, item) {
                    ...
                    if(item.text) ddOption.append($("<div>").addClass("dd-option-text").text(item.text));
    ...
    function selectIndex(obj, index, callbackOnSelection) {
        ...
        if (!pluginData) return;	// cryptostorage modification; prevents exception below but still works
        ...
        if (settings.showSelectedHTML) {
            ...
            if(selectedData.text) ddSelectedData.append($("<div>").addClass("dd-selected-text").text(selectedData.text));
